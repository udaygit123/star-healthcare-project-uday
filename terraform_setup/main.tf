resource "aws_instance" "Test-Server2" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      command ="ansiblePlaybook installation: 'ansible', inventory: '/etc/ansible/hosts', playbook: '/var/lib/jenkins/workspace/Health-care-Project/terraform_setup/deployment.yml"
    ]
    connection {
      type        = "ssh"
      host        = self.public_ip
