resource "aws_instance" "test-server2" {
 ...
 provisioner "local-exec" {
 command =  "ansiblePlaybook installation: 'ansible', inventory: '/etc/ansible/hosts', playbook: '/var/lib/jenkins/workspace/Health-care-Project/terraform_setup/ansible-playbook.yml"
   } 
}
